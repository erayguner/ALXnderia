...
6. **The `source_ip` field is not recorded in audit entries.** The `AuditEntry` interface and `recordAuditEntry()` function do not currently capture client IP addresses. Production must add this field and extract the real client IP from request headers (respecting proxy configuration). Additionally, `recordAuditEntry()` currently uses `pool.query()` directly (not `executeWithTenant()`), which means it does not set the tenant context for RLS. If the INSERT fails due to RLS, audit entries are silently dropped (caught by the `.catch(() => {})` handler). This should be reviewed for production use.
